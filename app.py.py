...
def load_data():
    ...
    # [핵심 로직: 데이터 전처리 및 지표 계산]
    # 1. 축제 전 기준값 (BEFORE_3M, 2M, 1M 평균)
    pre_period = ['BEFORE_3M', 'BEFORE_2M', 'BEFORE_1M']
    pre_avg = df_reg[df_reg['period'].isin(pre_period)].groupby(['festival_name', 'festival_year'])['외지인방문자수'].mean().reset_index()
    pre_avg.rename(columns={'외지인방문자수': 'pre_avg_visitor'}, inplace=True)
    
    # 2. 축제 당월 데이터 (FESTIVAL)
    fest_curr = df_reg[df_reg['period'] == 'FESTIVAL'][['festival_name', 'festival_year', '외지인방문자수']]
    fest_curr.rename(columns={'외지인방문자수': 'curr_visitor'}, inplace=True)
    
    # 3. 지속성 데이터 (AFTER_6M)
    after_6m = df_reg[df_reg['period'] == 'AFTER_6M'][['festival_name', 'festival_year', '외지인방문자수']]
    after_6m.rename(columns={'외지인방문자수': 'after_6m_visitor'}, inplace=True)
    
    # 데이터 병합
    m_df = pd.merge(pre_avg, fest_curr, on=['festival_name', 'festival_year'])
    m_df = pd.merge(m_df, after_6m, on=['festival_name', 'festival_year'])
    
    # 축제 특성 데이터(외지인 비율 계산용)와 병합
    m_df = pd.merge(m_df, df_fest[['festival_name', 'festival_year', '외지인방문자수', '전체방문자수']], 
                    left_on=['festival_name', 'festival_year'], right_on=['festival_name', 'festival_year'])
    ...
    def classify_type(row):
        if row['외지인_증감률'] > 0 and row['축제_외지인비율'] > 50: return "🌍 지역 경제활성화형"
        elif row['외지인_증감률'] > 0: return "🔶 혼합형"
        else: return "🏘️ 동네 잔치형"
...
# --- [3. 사이드바 메뉴] ---
menu = st.sidebar.radio("🔍 메뉴 선택", ["전체 개요", "효과성 분석", "지속성 분석", "종합 순위", "축제별 상세 분석"])
...
if menu == "전체 개요":
    ...
    col3.metric("지속효과 보유", f"{len(df[df['지속성_유지율'] >= 100])}개", delta=f"{len(df[df['지속성_유지율'] >= 100])/len(df)*100:.1f}%")

    c1, c2 = st.columns(2)
    with c1:
        st.subheader("축제 유형 분포")
        fig = px.pie(df, names='축제유형', hole=0.4, color_discrete_sequence=['#10B981', '#3B82F6', '#EF4444'], template="plotly_dark")
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        st.subheader("지속성 등급 분포")
        fig = px.bar(df['지속성등급'].value_counts().reset_index(), x='지속성등급', y='count', color='지속성등급', template="plotly_dark")
        st.plotly_chart(fig, use_container_width=True)
    
    st.info("💡 **인사이트:** 전체 축제 중 약 XX%가 경제 활성화 효과를 거두었으며, 지속성 측면에서는 🏆 등급이 가장 적은 비중을 차지합니다.")
...
# ② 효과성 분석
    ...
    fig = px.bar(top10, x='외지인_증감률', y='festival_name', orientation='h',
                 color='외지인_증감률', color_continuous_scale=['#EF4444', '#10B981'], template="plotly_dark")
    st.plotly_chart(fig, use_container_width=True)
    ...
    col1.dataframe(plot_df[plot_df['외지인_증감률'] > 0][['festival_name', '외지인_증감률']], use_container_width=True)
    col2.error("❌ 유입 효과 없음 (증감률 ≤ 0)")
    col2.dataframe(plot_df[plot_df['외지인_증감률'] <= 0][['festival_name', '외지인_증감률']], use_container_width=True)
...
# ③ 지속성 분석
    ...
    fig = px.bar(top10, x='지속성_유지율', y='festival_name', orientation='h', color_discrete_sequence=['#3B82F6'], template="plotly_dark")
    st.plotly_chart(fig, use_container_width=True)

    st.code("logic: AFTER_6M_외지인 / 축제전_평균 * 100")

    grades = plot_df['지속성등급'].unique()
    cols = st.columns(4)
    for i, g in enumerate(["🏆 강한 지속 효과", "✅ 보통 지속 효과", "🔶 약한 회귀", "❌ 강한 회귀"]):
        cols[i].write(f"**{g}**")
        cols[i].dataframe(plot_df[plot_df['지속성등급'] == g][['festival_name']], hide_index=True)
...
# ④ 종합 순위
    ...
    rank_df = df.sort_values('종합점수', ascending=False)[['festival_name', 'festival_year', '외지인_증감률', '지속성_유지율', '종합점수']]
    st.write("🔝 **TOP 5**")
    ...
    fig = px.scatter(df, x='외지인_증감률', y='지속성_유지율', color='축제유형', size='종합점수',
                     hover_name='festival_name', template="plotly_dark",
                     color_discrete_map={"🌍 지역 경제활성화형": "#10B981", "🔶 혼합형": "#F59E0B", "🏘️ 동네 잔치형": "#EF4444"})
    st.plotly_chart(fig, use_container_width=True)
...
# ⑤ 축제별 상세 분석
    ...
    order = ['BEFORE_3M', 'BEFORE_2M', 'BEFORE_1M', 'FESTIVAL', 'AFTER_1M', 'AFTER_2M', 'AFTER_3M', 'AFTER_6M']
    ...
    fig = px.line(flow_df.sort_values('period'), x='period', y='외지인방문자수', color='festival_year', 
                  markers=True, template="plotly_dark", title="시기별 방문자수 변화")
    st.plotly_chart(fig, use_container_width=True)
